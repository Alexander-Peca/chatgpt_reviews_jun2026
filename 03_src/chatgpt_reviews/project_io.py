...
def read_csv_from_processed(filename: str | Path, **kwargs) -> pd.DataFrame:
    """Read a CSV located in 01_data/processed."""
    return pd.read_csv(PROC_DIR / filename, **kwargs)


def read_excel_from_processed(filename: str | Path, **kwargs) -> pd.DataFrame:
    """Read an Excel file located in 01_data/processed."""
    return pd.read_excel(PROC_DIR / filename, **kwargs)
...
def write_table(
    ...
    **kwargs
    ...
    """Save a final table into 05_outputs/tables and return the output path."""
